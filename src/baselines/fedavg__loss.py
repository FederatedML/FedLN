...
parser.add_argument('--loss_t1',				type=float, default=0.8,				required=False)
parser.add_argument('--loss_t2',				type=float, default=1.4,				required=False)
...
def create_client(cid):
	...
	#############################################
	# Override Client to use Bi-Tempered loss   #
	#############################################
	import tensorflow as tf
	from utils.bt_loss import BiTemperedLogisticLoss

	class BTLossClient(Client):
		def __init__(self, *args, **kwargs):
			super(BTLossClient, self).__init__( *args, **kwargs)
			self.t1, self.t2 = kwargs['loss_t1'], kwargs['loss_t2']
		...
		def set_parameters(self, parameters, config):
			...
			self.model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=config['lr']), loss=BiTemperedLogisticLoss(t1=self.t1, t2=self.t2, name='loss', from_logits=True), metrics=[tf.keras.metrics.CategoricalAccuracy(name='accuracy')])
	...
	return BTLossClient(cid=cid, num_clients=int(args.num_clients),
					loss_t1=float(args.loss_t1), loss_t2=float(args.loss_t2), 
					model_loader=load_model, data_loader=load_train_data, **kwargs)
